...
//
G4VParticleChange*
G4OpRaman::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
        ...
        G4double CosTheta, SinTheta, SinPhi, CosPhi, unit_x, unit_y, unit_z, old_energy, old_lambda, new_energy, new_lambda;
        ...
        do {
           ...
           //get energy 
           old_energy = aParticle->GetKineticEnergy();
           //calculate the wavelength
	         old_lambda = 1.e3 * 6.242e18 * 6.62607e-34 * 2.99792e8 / old_energy;
           //calculate new wavelength due to scatter
	         new_lambda = 1.e7 / ((1.e7 / old_lambda) - 3400.);
           //calculate new energy due to scatter
	         new_energy = 1.e3 * 6.242e18 * 6.62607e-34 * 2.99792e8 / new_lambda;
	         //apply energy shift
           if(new_energy > 0){
             aParticleChange.ProposeEnergy(new_energy);
             } 
...
// --------------------------------------------------------
void G4OpRaman::BuildPhysicsTable(const G4ParticleDefinition&)
{
  if (thePhysicsTable) {
     thePhysicsTable->clearAndDestroy();
     delete thePhysicsTable;
...
// -----------------
//
G4double G4OpRaman::GetMeanFreePath(const G4Track& aTrack,
                                       G4double ,
                                       G4ForceCondition* )
{
...
G4OpRaman::CalculateRamanMeanFreePaths( const G4Material* material ) const
{
  G4MaterialPropertiesTable* materialProperties = 
                                       material->GetMaterialPropertiesTable();
  ...
  const G4double c1 = scaleFactor * betat * temperature * k_Boltzmann / 
                      ( 6.0 * pi );
  std::cout << "mean free path calc entered above" << std::endl;
  for( size_t uRIndex = 0; uRIndex < rIndex->GetVectorLength(); uRIndex++ )
  {
     ...
     const G4double meanFreePath = 0.5 * 1.0 / ( c1 * c2 * c3 );
     std::cout << "mean free path calc entered" << std::endl;
     if( verboseLevel>0 )
